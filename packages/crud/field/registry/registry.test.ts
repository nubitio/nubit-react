...
function entity(over: Partial<Field> = {}): Field {
  return {
    ...entityField('/api/categories', 'id', 'name').name('category').label('Category').build(),
    ...over,
  };
}
...
describe('filter term expansion', () => {
  ...
  it('expands a DATE between into a plain >=/<= pair', () => {
    expect(
      getFieldTypeModule(FieldType.DATE).buildFilterTerms(
        dateF,
        'between',
  ...
  it('drops missing edges of a DATE between', () => {
    expect(
      getFieldTypeModule(FieldType.DATE).buildFilterTerms(dateF, 'between', '|2026-01-31'),
    ).toEqual([['soldOn', '<=', '2026-01-31']]);
  });

  it('expands a DATETIME equals into a whole-day range', () => {
    expect(
      getFieldTypeModule(FieldType.DATETIME).buildFilterTerms(datetimeF, '=', '2026-01-15'),
    ).toEqual([
  ...
  it('bounds a DATETIME between with day edges', () => {
    expect(
      getFieldTypeModule(FieldType.DATETIME).buildFilterTerms(
        datetimeF,
        'between',
        '2026-01-01|2026-01-31',
      ),
...
describe('cell text', () => {
  it('formats currency with two decimals', () => {
    ...
    const expected = (1234.5).toLocaleString(getCoreLocale(), {
  ...
  it('resolves ENUM and SWITCH values through field.data', () => {
    const f = enumField([{ value: 'open', text: 'Open' }])
      .name('status')
      .label('Status')
      .build();
  ...
  it('shows entity display text from an object value', () => {
    const f = entity();
    expect(getFieldTypeModule(FieldType.ENTITY).cellText(f, { name: 'Books' }, cellCtx)).toBe(
      'Books',
    );
  });
  ...
  it('resolves an entity IRI through pre-loaded options', () => {
    ...
    expect(
      getFieldTypeModule(FieldType.ENTITY).cellText(f, '/api/categories/7', {
        ...cellCtx,
...
describe('form value serialization', () => {
  it('DATE keeps YYYY-MM-DD strings and omits empties', () => {
    ...
    expect(mod.serializeFormValue(f, '2026-05-02T10:00:00', ctx)).toEqual({
      kind: 'set',
      value: '2026-05-02',
    });
  });
  ...
  it('NUMBER omits empties, coerces values, and never touches identity keys', () => {
    ...
    expect(mod.serializeFormValue({ ...f, isIdentity: true }, 'uuid-1', ctx)).toEqual({
      kind: 'keep',
    });
  });
  ...
  it('CURRENCY serializes sendAsString values as fixed-point strings', () => {
    const f = currencyField().name('price').label('Price').build();
    expect(getFieldTypeModule(FieldType.CURRENCY).serializeFormValue(f, '12.3', ctx)).toEqual({
      kind: 'set',
      value: '12.30',
    });
  });
  ...
  it('ENTITY with multiple wraps scalars and maps arrays', () => {
    ...
    expect(mod.serializeFormValue(f, 3, ctx)).toEqual({
      kind: 'set',
      value: ['/api/categories/3'],
    });
    expect(mod.serializeFormValue(f, '', ctx)).toEqual({ kind: 'omit' });
  });
  ...
  it('NONE strips non-identity fields and keeps the identity', () => {
    ...
    expect(mod.serializeFormValue({ ...f, type: FieldType.NONE }, 'x', ctx)).toEqual({
      kind: 'omit',
    });
    expect(
      mod.serializeFormValue({ ...f, type: FieldType.NONE, isIdentity: true }, 7, ctx),
    ).toEqual({ kind: 'keep' });
  });
  ...
  it('FILE drops raw arrays in JSON mode and sends the first file in multipart mode', () => {
    ...
    const multipart: SerializeFieldContext = {
      ...ctx,
      format: 'multipart',
      getFieldValue: () => [file],
    };
...
describe('form width', () => {
  ...
  it('ENTITY is compact only for single refs', () => {
    ...
    expect(getFieldTypeModule(FieldType.ENTITY).formWidth?.(entity({ multiple: true }))).toBe(
      'auto',
    );
  });
...
describe('form value normalization', () => {
  const normCtx = (
    prepends: Map<string, Record<string, unknown>[]> = new Map(),
  ...
  it('FILE values are always omitted from the editor row', () => {
    const f = textField().name('doc').label('Doc').build();
    expect(
      getFieldTypeModule(FieldType.FILE).normalizeFormValue?.(f, '/api/media/1', normCtx()),
    ).toEqual({ kind: 'omit' });
  });
  ...
  it('PASSWORD values are always blanked', () => {
    ...
    expect(
      getFieldTypeModule(FieldType.PASSWORD).normalizeFormValue?.(f, '$hash', normCtx()),
  ...
  it('DATE truncates strings, formats Date objects, and nulls the rest', () => {
    const f = dateField().name('soldOn').label('Sold').build();
    const mod = getFieldTypeModule(FieldType.DATE);
    expect(mod.normalizeFormValue?.(f, '2026-05-02T10:00:00', normCtx())).toEqual({
      kind: 'set',
      value: '2026-05-02',
    });
  ...
  it('ENTITY resolves objects to their key and registers them as options', () => {
    ...
    expect(prepends.get('category')).toEqual([
      { '@id': '/api/categories/7', id: 7, name: 'Books' },
    ]);
  });
...
describe('detail value validation', () => {
  it('NUMBER and CURRENCY reject NaN in required cells and enforce range validators', () => {
    for (const type of [FieldType.NUMBER, FieldType.CURRENCY]) {
      ...
      const ranged = {
        ...f,
        validators: [{ type: 'range', options: { min: 1, max: 10 } }],
      } as Field;
