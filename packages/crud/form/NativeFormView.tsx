...
import {
  buildFormLayoutModel,
  type FormLayoutGroup,
  type FormLayoutModel,
} from './FormLayoutModel';
import {
...
function resolveDetailColWidth(
  field: Field,
  ...
  if (field.width !== undefined) {
    const parsed =
      typeof field.width === 'number' ? field.width : Number.parseInt(String(field.width), 10);
...
function DetailSummaryFooter({
  ...
  const itemsByColumn = new Map(
    summary.items.filter((item) => item.column).map((item) => [item.column, item]),
  );
                ...
                >
                  {item && (
                    <div className="nb-form__detail-summary-cell" style={{ alignItems }}>
                      {item.label && (
                        <span className="nb-form__detail-summary-label">{item.label}</span>
                      )}
                      <span className="nb-form__detail-summary-value">
...
function NativeDetailGrid({
  ...
  const handleResizeMouseDown = (fieldName: string) => (e: React.MouseEvent) => {
    ...
    const onMouseMove = (ev: MouseEvent) => {
      ...
      const next = Math.max(
        36,
        resizingRef.current.startWidth + (ev.clientX - resizingRef.current.startX),
      );
  ...
  const updateCell = async (rowIndex: number, field: Field, value: unknown) => {
    ...
    if (field.onChange) {
      ...
      await (field.onChange as (...args: unknown[]) => void | Promise<void>).call(
        fieldContext,
        draft,
        value,
        rows[rowIndex],
          ...
          <table className="nb-form__detail-table">
            ...
            <thead>
              <tr>
                {visibleFields.map((field) => (
                  <th key={field.name} style={field.align ? { textAlign: field.align } : undefined}>
            ...
            <tbody>
              {rows.length === 0 && (
                <tr>
                  <td
                    colSpan={visibleFields.length + (allowDeleting ? 1 : 0)}
                    className="nb-form__detail-empty"
                  >
                    ...
                    {allowAdding && (
                      <span className="nb-form__detail-empty-hint">
                        {t('form.detailEmptyHint')}
                      </span>
                    )}
                  </td>
                </tr>
                  ...
                  <tr key={rowKey}>
                    {visibleFields.map((field) => (
                      ...
                      >
                        ...
                        {detailErrors[rowIndex]?.[field.name] && (
                          <span className="nb-form__error">
...
export const NativeFormView = forwardRef<FormHandle, FormViewOptions>((options, ref) => {
  ...
  const computedFieldNames = useMemo(
    () => fields.filter((field) => field.computed !== undefined).map((field) => field.name),
    [fields],
  );
  const viewportWidth = useViewportWidth();
  const formLayoutModel = useMemo(
    () => buildFormLayoutModel(fields, options.formLayout, t('form.groupOther')),
    [fields, options.formLayout, t],
  );
  const optionFields = useMemo(
    () => [...fields, ...(options.detailFields ?? [])],
    [fields, options.detailFields],
  );
  const baseColSpanContext = useMemo<FieldColSpanContext>(
    () =>
      buildFieldColSpanContext({
    ...
    [formLayoutModel.type, options.detailFields, options.presentationContext, viewportWidth],
  );
  ...
  useEffect(() => {
    optionFields.forEach((field) => {
      ...
      const source = createRemoteSource(
        ...
        accessors.getPrependData(field.name),
      );
      source
        .load({ take: 50 })
        .then((result) => {
          ...
          if (
            field.autoSelectIfSingle &&
            result.data.length === 1 &&
            isEmptyValue(accessors.getFieldValue(field.name))
          ) {
  ...
  useEffect(() => {
    const subs = [
      on<unknown>(scopedFormErrorsEvent, (formErrors) => {
        ...
        notify(
          mapped.unassigned.length > 0 ? mapped.unassigned.join(' ') : t('form.validationError'),
          'error',
        );
        setErrors((current) => {
  ...
  useEffect(() => {
    ...
    computedFieldNames.forEach((name) => {
      if (
        Object.prototype.hasOwnProperty.call(options.computedValues, name) &&
  ...
  const renderField = (field: Field, span: ColSpan) => {
    ...
    return (
      <div key={field.name} className="nb-form__field" style={colSpanStyle(span)}>
        {!typeModule.rendersOwnLabel && (
          <label
            className={`nb-form__label${field.required ? ' nb-form__label--required' : ''}`}
            htmlFor={`nb-form-${field.name}`}
          >
            {field.label}
          </label>
        )}
  ...
  const renderLayout = (layout: FormLayoutModel) => {
    ...
    if (layout.type === 'sections') {
      return (
        <>
          {layout.sections.map(renderGroup)}
          {layout.overflow && renderGroup(layout.overflow)}
        </>
      );
    }
