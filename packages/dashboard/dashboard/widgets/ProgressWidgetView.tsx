...
export function ProgressWidgetView({ widget, data, loading }: Props) {
  ...
  const max =
    widget.maxPath !== undefined
      ? toNumber(resolvePath(data, widget.maxPath), 100)
      : (widget.max ?? 100);
  ...
  return (
    ...
    >
      <div
        className="nb-dashboard-progress__meter"
        role="progressbar"
        aria-valuenow={pct}
        aria-valuemin={0}
        aria-valuemax={100}
      ...
      <div className="nb-dashboard-progress__labels">
        <span className="nb-dashboard-progress__value">
          {formatDashboardValue(value, widget.format ?? 'number')}
        </span>
